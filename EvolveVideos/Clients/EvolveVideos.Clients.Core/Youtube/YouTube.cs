...
namespace EvolveVideos.Clients.Core.Youtube
{
    ...
    public class YouTubeUri
    {
        ...
        public Uri Uri
        {
            get { return new Uri(_url, UriKind.Absolute); }
        }
        ...
        public YouTubeQuality VideoQuality
        {
            get
            {
                switch (Itag)
                {
                    ...
                    case 85:
                        return YouTubeQuality.Quality520P;
                    ...
                    // audio only
                    case 139:
                        return YouTubeQuality.NotAvailable;
        ...
        public YouTubeQuality AudioQuality
        {
            get
            {
                switch (Itag)
                {
                    // video & audio
                    case 5:
                        return YouTubeQuality.QualityLow;
                    ...
                    case 18:
                        return YouTubeQuality.QualityMedium;

                    case 22:
                        return YouTubeQuality.QualityHigh;
    ...
    /// </summary>
        ...
        public static YouTubeUri GetBestVideoUri(IEnumerable<YouTubeUri> uris, YouTubeQuality minQuality,
            YouTubeQuality maxQuality)
        {
            return uris
                .Where(
                    u =>
                        u.HasVideo && u.HasAudio && !u.Is3DVideo && u.VideoQuality >= minQuality &&
                        u.VideoQuality <= maxQuality)
        ...
        /// </summary>
        /// <returns>Returns null when no appropriate URI has been found. </returns>
        public static Task<YouTubeUri> GetVideoUriAsync(string youTubeId, YouTubeQuality minQuality,
            YouTubeQuality maxQuality)
        {
        ...
        /// </summary>
        /// <returns>Returns null when no appropriate URI has been found. </returns>
        public static async Task<YouTubeUri> GetVideoUriAsync(string youTubeId, YouTubeQuality minQuality,
            YouTubeQuality maxQuality, CancellationToken token)
        {
        ...
        public static async Task<YouTubeUri[]> GetUrisAsync(string youTubeId, CancellationToken token)
        {
            ...
            var arr = Regex.Split(data + "," + data2, ",(?=(?:[^\"]*\"[^\"]*\")*[^\"]*$)");
            // split by comma but outside quotes
        ...
        private static string GenerateSignature(string signature, string javaScriptCode)
        {
            ...
            var functionMath = Regex.Match(javaScriptCode,
                "function " + Regex.Escape(functionName) + "\\((\\w+)\\)\\{(.+?)\\}", RegexOptions.Singleline);
            ...
            foreach (var line in functionBody.Split(';').Select(l => l.Trim()))
            {
                if (Regex.IsMatch(line, parameterName + "=" + parameterName + "\\.reverse\\(\\)")) // OLD
                    ...
                    signature = Slice(signature,
                        Convert.ToInt32(
                            Regex.Match(line, parameterName + "=" + parameterName + "\\.slice\\((\\d+)\\)").Groups[1]
                                .ToString()));
                else if (Regex.IsMatch(line, parameterName + "=\\w+\\(" + parameterName + ",\\d+\\)"))
                    signature = Swap(signature,
                        Convert.ToInt32(
                            Regex.Match(line, parameterName + "=\\w+\\(" + parameterName + ",(\\d+)\\)").Groups[1]
                                .ToString()));
                else if (Regex.IsMatch(line,
                    parameterName + "\\[0\\]=" + parameterName + "\\[\\d+%" + parameterName + "\\.length\\]"))
                    signature = Swap(signature,
                        Convert.ToInt32(
                            Regex.Match(line,
                                parameterName + "\\[0\\]=" + parameterName + "\\[(\\d+)%" + parameterName +
                                "\\.length\\]").Groups[1].ToString()));
                else
                {
                    ...
                    if (match.Success)
                    {
                        ...
                        if (methods == null)
                        {
                            ...
                            var code =
                                Regex.Match(javaScriptCode, "var " + root + "=\\{(.*?)\\};function").Groups[1]
                                    .ToString();
